...
```{r code, echo = F, warning = F, error = F, message = F, results = 'hide' }
library(tidyverse)
library(tm)
library(xml2)
library(rvest)
...
yco_init_hist <- yco_init %>% 
  ...
  labs(x = "words per company description", y = "frequency",
       title = "Figure 1 - histogram of words per company description, pre-processing")
...
yco_post_hist <- yco_post %>% 
  ...
  geom_histogram() + 
  labs(x = "words per company description", y = "frequency",
       title = "Figure 2 - histogram of words per company description, post-processing")
...
funding_summary_plot <- ggplot(funding_summary, 
  ...
  labs(x = "funding / status category",
       title = "Figure 2 - company funding and status statistics")
...
```{r regression, message = F, echo = F}

require(broom)
r_table_active <- log_active %>% 
  tidy() %>% 
  kbl() %>% 
  kable_classic()
r_table_active
